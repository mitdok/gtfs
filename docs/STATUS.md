...
| RT-5 | ⏳ 鮮度SLO表示＋stale policy＋監査ログMVP＋公開URL smoke＋静的GTFS参照ID照合API/Web＋source revision照合MVP完了 / revision切替予約未 | `/rt/status`、Web Freshness、`/rt/audit`、Web Audit、`/rt/smoke`、Web Smoke、`/rt/static-compat/check`、`/rt/sources/:id/static-compat/check`、Web Static Compat/Source compatでSLO逸脱・操作履歴・公開URL decode・RT参照ID整合を確認 |
