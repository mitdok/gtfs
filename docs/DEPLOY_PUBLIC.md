# 公開デプロイ手順・注記（gtfs / vps-sakura）

最終更新: 2026-08-05

main の最新ビルドを、公開VPS `vps-sakura`（`dokasen.com`）へ **`/gtfs/`（フロント）＋
`/gtfs-api/`（API）** として公開する。旧 `/gtfs2/` と `/gtfs2-api/` は廃止し、公開系統を
このURLへ一本化する。
...
Internet ──443(TLS)──> nginx (dokasen.com)
   ├─ /gtfs/        → 静的配信 /var/www/html/gtfs/             （フロント: React+MapLibre）
   └─ /gtfs-api/    → proxy 127.0.0.1:8787                     （API: systemd --user gtfs-api）
                          │  node:http（依存ゼロ＋core）
                          ▼
                       /var/www/gtfs-api/  （core/api dist ＋ node_modules ＋ data/）
```

- フロント: `https://dokasen.com/gtfs/`（vite `base=/gtfs/`、静的SPA、client-sideで取込/編集/検証/再出力が完結）
- API: `https://dokasen.com/gtfs-api/`（フロントと同一オリジンなので mixed-content/CORS問題なし）
...
`.github/workflows/deploy.yml` が、`main` のCI成功後にテスト済みcommitをVPSへデプロイする。
フロント、API、ソースzipを更新し、APIサービス再起動後に公開URLをsmoke testする。

VPS上では専用ラベル `gtfs-production` のself-hosted runnerを `ubuntu` ユーザーで常駐させる。
公開PRのジョブには使わず、既定ブランチ上のDeploy workflowだけを実行する。VPSのSSHポートを
GitHub Hosted Runnerへ公開せず、runnerからGitHubへの外向き接続だけで更新する。
...
rsync -av --delete --exclude 'dl/' packages/web/dist/ ubuntu@vps-sakura:/var/www/html/gtfs/
...
> 注: リポジトリのvite `base` は `/gtfs/`。API接続先はデプロイ時に
> `VITE_GTFS_API_BASE_URL=https://dokasen.com/gtfs-api` を指定する。
...
| サイト | 版 | 配布URL |
|--------|----|---------|
| `/gtfs/` | main | `https://dokasen.com/gtfs/dl/gtfs-studio-src-gtfs-<sha>.zip` |
...
- `dl/` は静的配信のみ。`/gtfs/dl/` はweb再デプロイの `rsync --delete` 対象内なので、上記の
  `--exclude 'dl/'` で保護し、デプロイ処理が最新版のzipへ入れ替える。
...
location = /gtfs2 { return 301 /gtfs/; }
location ^~ /gtfs2/ { return 301 /gtfs/; }
location = /gtfs2-api { return 410; }
location ^~ /gtfs2-api/ { return 410; }
```
