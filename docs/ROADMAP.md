...
| GTFS-RT | 約93% / Beta手前 | ServiceAlerts、RT中継＋source運用UI、VehiclePositions地図デバッグ、TripUpdates core/API MVP、trip候補・block_id/GPS絞り込み・進捗推定、車両位置連動MVP、候補評価JSON/CSV入力/結果表示/出力、鮮度SLO表示、stale policy、監査ログMVP、公開URL smoke、静的GTFS参照ID照合API/Web、source revision照合MVP | revision切替予約、実データ評価 |
