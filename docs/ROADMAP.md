...
| GTFS-RT | 約91% / Beta手前 | ServiceAlerts、RT中継＋source運用UI、VehiclePositions地図デバッグ、TripUpdates core/API MVP、trip候補・進捗推定、車両位置連動MVP、GPS最寄り停留所絞り込み、候補評価JSON/CSV入力/結果表示/出力、鮮度SLO表示、stale policy、監査ログMVP、公開URL smoke、静的GTFS参照ID照合API/Web | revision切替運用、実データ評価 |
